const asyncHandler = require('express-async-handler');
const Accessory = require('../models/Accessory.js');
const User = require('../models/User.js');
const mongoose = require('mongoose');
const slugify = require('slugify');
const { cloudinary } = require('../utils/cloudinary');

// @desc    Fetch all accessories
// @route   GET /api/accessories
// @access  Public
const getAccessories = asyncHandler(async (req, res) => {
  const pageSize = 12;
  const page = Number(req.query.pageNumber) || 1;
  const keyword = req.query.keyword 
    ? { name: { $regex: req.query.keyword, $options: 'i' } }
    : {};
  const categoryFilter = req.query.category ? { category: req.query.category } : {};

  const count = await Accessory.countDocuments({ ...keyword, ...categoryFilter });
  const accessories = await Accessory.find({ ...keyword, ...categoryFilter })
    .populate('compatibleWith.product', 'name slug brand')
    .limit(pageSize)
    .skip(pageSize * (page - 1))
    .sort({ createdAt: -1 });

  res.json({ accessories, page, pages: Math.ceil(count / pageSize) });
});

// @desc    Fetch single accessory by ID
// @route   GET /api/accessories/:id
// @access  Public
const getAccessoryById = asyncHandler(async (req, res) => {
  const accessory = await Accessory.findById(req.params.id)
    .populate('compatibleWith.product', 'name slug brand images');

  if (accessory) {
    res.json(accessory);
  } else {
    res.status(404);
    throw new Error('Accessory not found');
  }
});

// @desc    Fetch single accessory by slug - FOR PRODUCT PAGE
// @route   GET /api/accessories/slug/:slug
// @access  Public
const getAccessoryBySlug = asyncHandler(async (req, res) => {
  const accessory = await Accessory.findOne({ slug: req.params.slug })
    .populate('compatibleWith.product', 'name slug brand images');

  if (accessory) {
    res.json(accessory);
  } else {
    res.status(404);
    throw new Error('Accessory not found');
  }
});

// @desc    Create new accessory - ADMIN
// @route   POST /api/accessories
// @access  Private/Admin
const createAccessory = asyncHandler(async (req, res) => {
  const { 
    name, 
    brand, 
    category, 
    description, 
    metaTitle,
    metaDescription,
    keywords = [],
    variants = [],
    compatibleWith = []
  } = req.body;

  // 1. VALIDATION
  if (!name || !brand) {
    res.status(400);
    throw new Error('Please add name and brand');
  }

  // 2. CLEAN/MAP VARIANTS
  const cleanVariants = variants
  .map(v => ({
    type: v.type || 'Color',
    value: v.value || '',
    specs: v.specs || {},
    description: v.description || '',
    options: (v.options || [])
      .filter(opt => opt.name)
      .map(opt => {
        const discount = {
          type: opt.discount?.type || "percentage",
          value: Number(opt.discount?.value) || 0,
          startDate: opt.discount?.startDate || null,
          endDate: opt.discount?.endDate || null,
        };
        discount.isActive = 
          discount.value > 0 &&
          (!discount.startDate || new Date() >= new Date(discount.startDate)) &&
          (!discount.endDate || new Date() <= new Date(discount.endDate));

        return {
          name: opt.name,
          hexCode: opt.hexCode || "",
          price: Number(opt.price) || 0,
          discount,
          countInStock: Number(opt.countInStock) || 0,
          sku: opt.sku || '',
          images: opt.images || [],
        }
      })
  }))
  .filter(v => v.options.length > 0);

  if (cleanVariants.length === 0) {
    res.status(400);
    throw new Error('Please add at least 1 variant with 1 option');
  }

  // 3. V9.47 KEY: SLUG + META FROM FIRST VARIANT - Same as Product
  const baseSlug = slugify(name, { lower: true, strict: true, remove: /[*+~.()'"!:@]/g });
  const accessorySlug = `${baseSlug}-${Date.now()}`;

  const autoMetaTitle = metaTitle || `${name} | ${brand}`;
  const autoMetaDescription = metaDescription || `Buy ${name} from ${brand}.`;

  // 4. CLEAN COMPATIBLE
  const cleanCompatible = (compatibleWith || []).map(c => ({
    product: c.product,
    countInStock: Number(c.countInStock) || 0,
    imagePublicId: c.imagePublicId || ''
  }));

  try {
    const accessory = new Accessory({
      user: req.user._id,
      name,
      slug: accessorySlug, // manual like product
      brand,
      category,
      description,
      metaTitle: autoMetaTitle.slice(0, 60),
      metaDescription: autoMetaDescription.slice(0, 155),
      keywords,
      variants: cleanVariants,
      compatibleWith: cleanCompatible,
      allSales: 0,
      numReviews: 0,
      rating: 0,
    });

    const createdAccessory = await accessory.save();
    res.status(201).json(createdAccessory);
  } catch (error) {
    console.error('REAL MONGOOSE ERROR:', error);
    res.status(400);
    throw new Error(error.message);
  }
});

// @desc    Update accessory - ADMIN
// @route   PUT /api/accessories/:id
// @access  Private/Admin
const updateAccessory = asyncHandler(async (req, res) => {
  const {
    name,
    brand,
    category,
    description,
    metaTitle,
    metaDescription,
    keywords,
    variants,
    compatibleWith,
    removedPublicIds = [], // array of cloudinary public_ids to delete
  } = req.body;

  const accessory = await Accessory.findById(req.params.id);

  if (accessory) {
    // 1. DELETE IMAGES FROM CLOUDINARY IF USER REMOVED THEM
    if (removedPublicIds && removedPublicIds.length > 0) {
      const idsArray = [...removedPublicIds];
      console.log(`Deleting ${idsArray.length} images from Cloudinary`);
      for (let i = 0; i < idsArray.length; i += 100) {
        const batch = idsArray.slice(i, i + 100);
        await cloudinary.api.delete_resources(batch);
      }
    }

    // 2. STORE OLD NAME TO CHECK IF WE NEED NEW SLUG
    const oldName = accessory.name;

    // 3. UPDATE BASIC FIELDS
    accessory.name = name || accessory.name;
    accessory.brand = brand || accessory.brand;
    accessory.category = category || accessory.category;
    accessory.description = description || accessory.description;

    if (keywords) accessory.keywords = keywords;

    // 4. CLEAN/MAP VARIANTS - SAME LOGIC AS CREATE
    if (variants) {
      const cleanVariants = variants
        .map((v) => ({
          type: v.type || 'Color',
          value: v.value || '',
          specs: v.specs || {},
          description: v.description || '',
          options: (v.options || [])
            .filter((opt) => opt.name)
            .map((opt) => {
              const discount = {
                type: opt.discount?.type || 'percentage',
                value: Number(opt.discount?.value) || 0,
                startDate: opt.discount?.startDate || null,
                endDate: opt.discount?.endDate || null,
                isActive: opt.discount?.isActive || false,
              };

              // Auto-calculate isActive
              discount.isActive =
                discount.value > 0 &&
                (!discount.startDate || new Date() >= new Date(discount.startDate)) &&
                (!discount.endDate || new Date() <= new Date(discount.endDate));

              return {
                name: opt.name,
                hexCode: opt.hexCode || '',
                price: Number(opt.price) || 0,
                countInStock: Number(opt.countInStock) || 0,
                sku: opt.sku || '',
                images: opt.images || [], // frontend sends merged: old images + new uploaded
                discount,
              };
            }),
        }))
        .filter((v) => v.options.length > 0);

      if (cleanVariants.length > 0) {
        accessory.variants = cleanVariants;
      }
    }

    // 5. CLEAN COMPATIBLE
    if (compatibleWith) {
      const cleanCompatible = compatibleWith.map((c) => ({
        product: c.product,
        countInStock: Number(c.countInStock) || 0,
        imagePublicId: c.imagePublicId || '',
      }));
      accessory.compatibleWith = cleanCompatible;
    }

    // 6. SEO META
    if (metaTitle) accessory.metaTitle = metaTitle.slice(0, 60);
    if (metaDescription) accessory.metaDescription = metaDescription.slice(0, 155);

    // 7. REGENERATE SLUG IF NAME CHANGED - SAME AS PRODUCT
    if (name && name !== oldName) {
      const baseSlug = slugify(name, {
        lower: true,
        strict: true,
        remove: /[*+~.()'"!:@]/g,
      });
      accessory.slug = `${baseSlug}-${Date.now()}`;
    }

    const updatedAccessory = await accessory.save();
    res.json(updatedAccessory);
  } else {
    res.status(404);
    throw new Error('Accessory not found');
  }
});

// @desc    Delete accessory - ADMIN
// @route   DELETE /api/accessories/:id
// @access  Private/Admin
const deleteAccessory = asyncHandler(async (req, res) => {
  const isDemoAdmin = req.user.email === 'demo@phonestore.com';
  if (isDemoAdmin) {
    return res.status(403).json({ message: 'Demo accounts have read-only access.' });
  }

  const accessory = await Accessory.findById(req.params.id);
  if (!accessory) {
    res.status(404);
    throw new Error('Accessory not found');
  }

  try {
    const publicIdsToDelete = new Set();

    // 1. COLLECT ALL OPTION IMAGE PUBLIC_IDS FROM VARIANTS
    accessory.variants?.forEach((variant) => {
      variant.options?.forEach((option) => {
        option.images?.forEach((img) => {
          if (img.imagePublicId) publicIdsToDelete.add(img.imagePublicId);
        });
      });
    });

    // 2. DELETE FROM CLOUDINARY IN BATCHES OF 100
    const idsArray = [...publicIdsToDelete];
    if (idsArray.length > 0) {
      console.log(`Deleting ${idsArray.length} accessory images from Cloudinary`);
      for (let i = 0; i < idsArray.length; i += 100) {
        const batch = idsArray.slice(i, i + 100);
        await cloudinary.api.delete_resources(batch);
      }
    }

    // 3. DELETE ACCESSORY FROM DB
    await accessory.deleteOne();

    // 4. CLEAN UP USER CARTS AND WISHLISTS
    const accessoryId = new mongoose.Types.ObjectId(req.params.id);
    await User.updateMany(
      { wishlist: accessoryId },
      { $pull: { wishlist: accessoryId }}
    );
    await User.updateMany(
      { 'cart.product': accessoryId },
      { $pull: { cart: { product: accessoryId } } }
    );

    res.json({ message: 'Accessory and all images removed' });

  } catch (error) {
    console.error('Delete accessory error:', error);
    res.status(500);
    throw new Error('Failed to delete accessory');
  }
});

// @desc    Create new review
// @route   POST /api/accessories/:id/reviews
// @access  Private
const createAccessoryReview = asyncHandler(async (req, res) => {
  const { rating, comment } = req.body;

  const accessory = await Accessory.findById(req.params.id);

  if (accessory) {
    const alreadyReviewed = accessory.reviews.find(
      (r) => r.user.toString() === req.user._id.toString()
    );

    if (alreadyReviewed) {
      res.status(400);
      throw new Error('Accessory already reviewed');
    }

    const review = {
      name: req.user.name,
      rating: Number(rating),
      comment,
      user: req.user._id,
    };

    accessory.reviews.push(review);
    accessory.numReviews = accessory.reviews.length;
    accessory.rating = accessory.reviews.reduce((acc, item) => item.rating + acc, 0) / accessory.reviews.length;

    await accessory.save();
    res.status(201).json({ message: 'Review added' });
  } else {
    res.status(404);
    throw new Error('Accessory not found');
  }
});

module.exports = {
  getAccessories,
  getAccessoryById,
  getAccessoryBySlug,
  createAccessory,
  updateAccessory,
  deleteAccessory,
  createAccessoryReview,
};